...
SAVE_FOLDER=lunaInit_hasAug_multiNeg_adam0.001_e20
KFOLD=28
GPU=0,1,2,3,4,5,6,7

for ((splitId=0; splitId<28; splitId+=1))
do
    echo "Start to run kfold splitId ${splitId}"
    python classify_aug.py --gpu=${GPU} --kfold=${KFOLD} --splitId=${splitId} --save_dir=results/${SAVE_FOLDER}/${KFOLD}fold --train=True -b=16 -e=20 \

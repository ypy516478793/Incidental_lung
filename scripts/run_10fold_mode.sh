...
do
    ...
    python classifier/classify.py --gpu=${GPU} --kfold=${KFOLD} --splitId=${splitId} \
      --save_dir=classifier/results/release_methodist/modeNorm_${KFOLD}fold --train=True -ts=0.1 \
      -p="./Methodist_incidental/data_Ben/modeNorm/"
done
# Evaluation
python classifier/evaluate_kfold.py --kfold=${KFOLD} \
    --save_dir=classifier/results/release_methodist/modeNorm_${KFOLD}fold
...
## Mode normalization 3
# 10 fold Training and testing
for ((splitId=0; splitId<${KFOLD}; splitId+=1))
do
    echo "Start to run kfold splitId ${splitId}"
    python classifier/classify.py --gpu=${GPU} --kfold=${KFOLD} --splitId=${splitId} \
      --save_dir=classifier/results/release_methodist/modeNorm3_${KFOLD}fold --train=True -ts=0.1 \
      -p="./Methodist_incidental/data_Ben/modeNorm3/"
done
# Evaluation
python classifier/evaluate_kfold.py --kfold=${KFOLD} \
    --save_dir=classifier/results/release_methodist/modeNorm3_${KFOLD}fold

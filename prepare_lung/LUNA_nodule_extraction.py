...
def get_node_classify():
    ...
    for subsetindex in range(10):
        ...
        output_path = "../LUNA16/cubes_64_newnewnew/"
        ...
        for fcount, img_file in tqdm(enumerate(file_list)):
            ...
            if mini_df.shape[0] > 0:
                ...
                index = 0
                mini_pos_df = mini_df[mini_df["class"] == 1]
                mini_neg_df = mini_df[mini_df["class"] == 0]
                mini_neg_df = mini_neg_df.sample(np.min([len(mini_neg_df), len(mini_pos_df)]))
                mini_df = pd.concat([mini_neg_df, mini_pos_df])
